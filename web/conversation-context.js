...
export function isContinuation(task=''){
  ...
  return /^(?:did|have|has|was|were|is|are|what|which)\b[\s\S]*\b(?:improvements?|plans?|changes?|executed|implemented|installed|verified|done|finished|working)\b/i.test(text)||isContextualFollowup(text)||/^(?:(?:ok|yes|please)[,\s]+)*(?:(?:do|execute|implement|apply|finish|continue|verify|fix|remember)\b.{0,80}\b(?:previous|these|those|this|that|our|chat history|recommendations?|plans?)\b|make\s+\d+\s+more\s+improvements?\b|(?:did|have|has|were)\b.{0,40}\bself[- ]?improvements?\b)/i.test(text);
...
export function requestsChanges(task='',context={}){
  ...
  return /\b(?:fix|repair|edit|patch|change)\b/i.test(target.replace(/\b(?:fix|repair)\s+(?:(?:any|the)\s+)?(?:failed|failing|broken)\s+tests?\b/ig,''))||/\b(?:improve|improvements?|implement|install|integrate|refactor|modify|add|recommendations?)\b/i.test(target);
}
