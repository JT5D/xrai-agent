...
async function applyEdits(wc,files,edits,progress=()=>{}){let applied=0;const changed=[];for(const e of edits.slice(0,8)){if(!e?.path||typeof e.search!=='string'||typeof e.replace!=='string'||e.search===e.replace)continue;let cur;try{cur=await wc.fs.readFile(e.path,'utf8')}catch{continue}if(!cur.includes(e.search)){progress(`Skipped ${e.path}: search text not found`);continue}const next=cur.replace(e.search,e.replace);await wc.fs.writeFile(e.path,next);const row=files.find(f=>f.path===e.path);if(row)row.text=next;else files.push({path:e.path,text:next});progress(`Edited ${e.path}`);changed.push({path:e.path,before:cur,after:next});applied++}return{applied,changed}}
...
export async function runBrowserRepoTask(task,opts={},emit=()=>{},progress=()=>{}){
  ...
  try{
    ...
    const summary=`${repo}@${snapshot.sha.slice(0,8)}: ${passed?'verified successfully':'verification still failing'}. ${checks}.${why}${edits?` ${finalChanges.length} file${finalChanges.length===1?'':'s'} changed in the browser sandbox.`:' No code edits were necessary.'}`;
    event('eval',`${passed?'Verified':'Not verified'} from real command exit codes`,{name:'Evidence gate',data:{score:passed?1:0,commands:evidence.map(x=>({cmd:x.cmd,exitCode:x.code,timedOut:x.timedOut}))}});event('run:done',summary,{data:{score:passed?1:0,attempts:edits?2:1,provider:`WebContainer + ${modelName}`,learning:'evidence',sha:snapshot.sha,changedFiles:finalChanges.map(x=>x.path)}});
    return{runId,output:summary,score:passed?1:0,attempts:edits?2:1,provider:`WebContainer + ${modelName}`,learning:{status:passed?'verified':'rejected'},evidence,repo,branch:snapshot.branch,sha:snapshot.sha,edits:finalChanges.length,diff,changedFiles:finalChanges.map(x=>x.path)};
  }finally{
