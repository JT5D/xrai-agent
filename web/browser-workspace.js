...
export async function runBrowserRepoTask(task,opts={},emit=()=>{},progress=()=>{}){
  ...
  try{
    ...
    const taskComplete=passed&&(!requestedChange||finalChanges.length>0),status=taskComplete?'completed':'incomplete';
    ...
    event('eval',`${passed?'Verified':'Not verified'} from real command exit codes`,{name:'Evidence gate',data:{score:passed?1:0,commands:evidence.map(x=>({cmd:x.cmd,exitCode:x.code,timedOut:x.timedOut}))}});
    event('run:done',summary,{data:{score:passed?1:0,attempts:edits?2:1,provider:`WebContainer + ${modelName}`,learning:'none',verification:passed?'sandbox-passed':'sandbox-failed',status,sha:snapshot.sha,changedFiles:finalChanges.map(x=>x.path)}});
    return{runId,status,output:summary,score:passed?1:0,attempts:edits?2:1,provider:`WebContainer + ${modelName}`,learning:{status:'none',reason:'Sandbox verification is execution evidence; no reusable learning was retained.'},verification:{status:passed?'sandbox-passed':'sandbox-failed',commands:evidence.map(x=>({cmd:x.cmd,exitCode:x.code,timedOut:x.timedOut}))},evidence,repo,branch:snapshot.branch,sha:snapshot.sha,edits:finalChanges.length,diff,changedFiles:finalChanges.map(x=>x.path)};
  }finally{
