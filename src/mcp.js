...
export const MCP_TOOLS=[
  {name:'xrai_run',description:'Run the complete recursive XRAI agent. Uses OpenAI when configured or a local Ollama model with no API key.',inputSchema:{type:'object',properties:{task:{type:'string'},workspace:{type:'string'}},required:['task'],additionalProperties:false}},
  ...
  {name:'xrai_knowledge',description:'Search XRAI knowledge and verified lessons.',inputSchema:{type:'object',properties:{query:{type:'string'},workspace:{type:'string'},limit:{type:'integer',minimum:1,maximum:8}},required:['query'],additionalProperties:false}}
];
...
export async function handleRpc(msg){
  const id=msg?.id;
  try{
    if(msg?.method==='initialize')return ok(id,{protocolVersion:msg.params?.protocolVersion||'2025-11-25',capabilities:{tools:{listChanged:false}},serverInfo:{name:'xrai-agent',version:'0.1.0'},instructions:'Use XRAI tools for workspace execution, XRAI knowledge retrieval, or the complete recursive agent.'});
    ...
    if(msg?.method==='tools/call'){
      ...
      if(name==='xrai_run'){const r=await runTask(a.task,{workspace:a.workspace});text=JSON.stringify({output:r.output,score:r.score,runId:r.runId},null,2)}
