...
export async function startWebServer(port=Number(process.env.XRAI_PORT||8787),host=process.env.XRAI_HOST||'127.0.0.1'){
  ...
  const server=createServer(async(req,res)=>{try{
    ...
    if(url.pathname==='/api/capabilities'){const local=process.env.OPENAI_API_KEY?{available:false}:await detectOllama();return json(res,200,{version:'0.2.0',learning:'evidence-gated-skills',autonomous:Boolean(process.env.OPENAI_API_KEY||local.available),provider:process.env.OPENAI_API_KEY?'openai':local.available?'ollama':'browser-local',model:process.env.OPENAI_API_KEY?(process.env.XRAI_MODEL||'gpt-5.6-luna'):(local.model||'on-device browser model'),mcp:'/mcp'})}
