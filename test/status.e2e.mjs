...
try{
  for(const width of [1440,390]){
    ...
    if(!live){
      ...
      await page.route('**/build-info.json',route=>route.fulfill({contentType:'application/json',body:JSON.stringify({repo:'JT5D/xrai-agent',sha,runId,version:'0.3.50'})}));
    ...
    if(!live){
      await page.evaluate(({sha})=>{
        ...
        localStorage.setItem('xrai-ui-v4',JSON.stringify(s));window.dispatchEvent(new Event('xrai:state-updated'));
      },{sha});
