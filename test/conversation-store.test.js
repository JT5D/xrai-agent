...
import { ACTIVE_CHAT_KEY,CHAT_SWITCH_KEY,CHATS_KEY,UI_STATE_KEY,listChats,newChat,restoreChat,snapshotChat } from '../web/conversation-store.js';
...
test('conversation store snapshots multiple chats and restores exact old progress atomically',()=>{
  ...
  const second=newChat(s);assert.notEqual(second,'chat-a');s.setItem(UI_STATE_KEY,JSON.stringify(state('second task','second result','run-b')));snapshotChat(s,state('second task','second result','run-b'),second);
...
test('switching back and forth preserves each chats messages events and result without leakage',()=>{
  ...
  session.removeItem(CHAT_SWITCH_KEY);assert.equal(restoreChat(s,'beta',session),true);restored=JSON.parse(s.getItem(UI_STATE_KEY));assert.deepEqual(restored.messages.map(x=>x.text),['beta','beta answer']);assert.equal(restored.events[0].runId,'beta-run');assert.doesNotMatch(JSON.stringify(restored),/alpha answer/);
});
...
test('missing chat does not disturb current state or arm a transition',()=>{
  ...
  assert.equal(restoreChat(s,'does-not-exist',session),false);assert.equal(JSON.parse(s.getItem(UI_STATE_KEY)).lastTask,'current');assert.equal(session.getItem(CHAT_SWITCH_KEY),null);
});
