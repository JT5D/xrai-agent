...
test('static Pages build has no-key local inference, retrieval, and no browser self-promotion loop',async()=>{
  ...
  assert.match(app,/runLocalTask/);assert.match(local,/LanguageModel/);assert.match(local,/LFM2\.5-350M-ONNX/);assert.match(local,/SmolLM2-135M-Instruct-ONNX-MHA/);assert.match(local,/xrai-skills-v2/);assert.match(local,/transferVerified===true/);assert.match(local,/RETRIEVED EVIDENCE/);assert.match(local,/learning:'none'/);assert.doesNotMatch(local,/supportNeeded|directPromoteScore|skill:promoted|orchestration:decision|parsePlannerDecision/);assert.match(bootstrap,/app\.js/);assert.match(bootstrap,/browser-enhancements\.js/);
});
...
test('browser diagnostic evaluator parse failure is fail-closed and never creates a skill',()=>{
...
test('browser diagnostic evaluator preserves bounded composite scoring without learning authority',()=>{
  const ev=extractEval('{"score":0.95,"pathScore":0.4,"critique":"good answer","pathCritique":"weak evidence","skill":{"title":"do not retain","trigger":"x","procedure":"y","verifier":"npm test","tags":[]}}');
  assert.equal(ev.score,.95);assert.equal(ev.pathScore,.4);assert.equal(ev.compositeScore,.55);assert.equal(ev.skill.title,'');
});

test('compounding metrics use only explicitly verified run records',()=>{
  const m=compoundingMetrics([
    {verified:false,passed:true,score:1,pathScore:1,attempts:1,skillRefs:['fake@1']},
    {verified:true,passed:true,score:.9,pathScore:.88,attempts:1,skillRefs:['skill-a@1']},
    {verified:true,passed:true,score:.86,pathScore:.84,attempts:2,skillRefs:['skill-a@1']},
    {verified:true,passed:true,score:.78,pathScore:.76,attempts:1,skillRefs:[]},
    {verified:true,passed:false,score:.8,pathScore:.79,attempts:2,skillRefs:[]}
  ]);
  assert.equal(m.window,4);assert.equal(m.skillRuns,2);assert.equal(m.noSkillRuns,2);assert.ok(m.observedSkillLift>.08);assert.equal(m.repairRuns,2);assert.equal(m.repairSuccessRate,.5);assert.equal(m.avgAttempts,1.5);assert.ok(m.avgPathScore>.8);
});
...
test('browser chooses a smaller quantized model on mobile, a full model on desktop, and a fast fallback when requested',()=>{
